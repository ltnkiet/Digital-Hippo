...
const ProductCart = ({ dispatch, navigate }) => {
  ...
  return (
    <div onClick={e => e.stopPropagation()} className='w-2/6 fixed h-screen bg-white grid grid-rows-10 text-black p-6'>
        ...
        {currentCart && currentCart?.map(el => (
          <div key={el._id} className='flex justify-between items-center'>
            <div className='flex gap-2'>
              ...
              <span onClick={() => removeCart(el.products?._id, el.color)} 
      ...
      <div className='row-span-2 flex flex-col justify-between h-full'>
        <div className='flex items-center justify-between pt-4 border-t'>
            <span>Tổng tiền:</span>
            <span>{formatPrice(currentCart?.reduce((sum, el) => sum + Number(el.price) * el.quantity, 0))}</span>
        </div>
        {/* <span className='text-center text-gray-700 italic text-xs'>Shipping, taxes, and discounts calculated at checkout.</span> */}
        <ButtonV2 handleOnClick={() => {
            dispatch(showCart())
            navigate(`/${path.MEMBER}/${path.DETAIL_CART}`)
        }} style='rounded-none w-full bg-main py-3' >Tiến hành thanh toán</ButtonV2>
      </div>
    </div>
  )
}
